 ...
 *
 */
if ( ! class_exists( 'ESHB_Field_media' ) ) {
  class ESHB_Field_media extends ESHB_Fields {
    ...
    public function render() {
      ...
      $args = wp_parse_args( $this->field, array(
        ...
        'preview_height' => '',
        'library'        => array(),
        'button_title'   => esc_html__( 'Upload', 'easy-hotel' ),
        'remove_title'   => esc_html__( 'Remove', 'easy-hotel' ),
      ...
      $placeholder = ( empty( $this->field['placeholder'] ) ) ? 'Upload' : '';
      ...
      if ( ! empty( $args['preview'] ) ) {
        ...
        echo '<div class="csf-image-preview"'. esc_attr($preview_style) .'>';
      ...
      echo '<input type="text" name="'. esc_attr( $this->field_name( '[url]' ) ) .'" value="'. esc_attr( $this->value['url'] ) .'" class="csf--url'. esc_attr( $hidden_url ) .'" readonly="readonly"'. esc_attr($this->field_attributes()) .' placeholder="'. esc_attr($placeholder) .'"/>';
      echo '<a href="#" class="button button-primary csf--button" data-library="'. esc_attr( $library ) .'" data-preview-size="'. esc_attr( $args['preview_size'] ) .'">'. esc_html($args['button_title']) .'</a>';
      echo ( empty( $args['preview'] ) ) ? '<a href="#" class="button button-secondary csf-warning-primary csf--remove'. esc_attr( $hidden_auto ) .'">'. esc_html($args['remove_title']) .'</a>' : '';
      echo '</div>';
