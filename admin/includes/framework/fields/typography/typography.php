 ...
 *
 */
if ( ! class_exists( 'ESHB_Field_typography' ) ) {
  class ESHB_Field_typography extends ESHB_Fields {
    ...
    public function render() {
      ...
      $args                  = wp_parse_args( $this->field, array(
        ...
        'custom_style'       => false,
        'compact'            => false,
        'exclude'            => '', // phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_exclude -- excluding unnecessary options.
        'unit'               => 'px',
        'line_height_unit'   => '',
      ...
      echo '<div class="csf--typography'. esc_attr( $chosen_class ) .'" data-depend-id="'. esc_attr( $this->field['id'] ) .'" data-unit="'. esc_attr( $args['unit'] ) .'" data-line-height-unit="'. esc_attr( $line_height_unit ) .'" data-exclude="'. esc_attr( $args['exclude'] ) .'">';
        ...
        echo '<div class="csf--blocks csf--blocks-selects">';
          ...
          if ( ! empty( $args['font_family'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Font Family', 'easy-hotel' ) .'</div>';
            echo esc_html($this->create_select( array( $this->value['font-family'] => $this->value['font-family'] )), 'font-family', esc_html__( 'Select a font', 'easy-hotel' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['backup_font_family'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Backup Font Family', 'easy-hotel' ) .'</div>';
            echo esc_html($this->create_select( apply_filters( 'eshb_field_typography_backup_font_family', array(
              ...
              'Georgia, serif',
              'Palatino Linotype'
            ) ), 'backup-font-family', esc_html__( 'Default', 'easy-hotel' ) ));
            echo '</div>';
          }
          ...
          if ( ! empty( $args['font_weight'] ) || ! empty( $args['font_style'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Font Style', 'easy-hotel') .'</div>';
            echo '<select class="csf--font-style-select" data-placeholder="Default">';
            echo '<option value="">'. ( ! $this->chosen ? esc_html__( 'Default', 'easy-hotel' ) : '' ) .'</option>';
            ...
            if ( ! empty( $args['extra_styles'] ) ) {
              ...
              echo ( ! $this->chosen ) ? '<div class="csf--title">'. esc_html__( 'Load Extra Styles', 'easy-hotel' ) .'</div>' : '';
              $placeholder = ( $this->chosen ) ? esc_html__( 'Load Extra Styles', 'easy-hotel' ) : esc_html__( 'Default', 'easy-hotel' );
              echo esc_html($this->create_select( $this->value['extra-styles'], 'extra-styles', $placeholder, true ));
              echo '</div>';
            }
          ...
          if ( ! empty( $args['subset'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Subset', 'easy-hotel' ) .'</div>';
            $subset = ( is_array( $this->value['subset'] ) ) ? $this->value['subset'] : array_filter( (array) $this->value['subset'] );
            echo esc_html($this->create_select( $subset, 'subset', esc_html__( 'Default', 'easy-hotel' ), $args['multi_subset'] ));
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_align'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Text Align', 'easy-hotel' ) .'</div>';
            echo esc_html($this->create_select( array(
              'inherit' => esc_html__( 'Inherit', 'easy-hotel' ),
              'left'    => esc_html__( 'Left', 'easy-hotel' ),
              'center'  => esc_html__( 'Center', 'easy-hotel' ),
              'right'   => esc_html__( 'Right', 'easy-hotel' ),
              'justify' => esc_html__( 'Justify', 'easy-hotel' ),
              'initial' => esc_html__( 'Initial', 'easy-hotel' )
            ), 'text-align', esc_html__( 'Default', 'easy-hotel' ) ));
            echo '</div>';
          }
          ...
          if ( ! empty( $args['font_variant'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Font Variant', 'easy-hotel' ) .'</div>';
            echo esc_html($this->create_select( array(
              'normal'         => esc_html__( 'Normal', 'easy-hotel' ),
              'small-caps'     => esc_html__( 'Small Caps', 'easy-hotel' ),
              'all-small-caps' => esc_html__( 'All Small Caps', 'easy-hotel' )
            ), 'font-variant', esc_html__( 'Default', 'easy-hotel' ) ));
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_transform'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Text Transform', 'easy-hotel' ) .'</div>';
            echo esc_html($this->create_select( array(
              'none'       => esc_html__( 'None', 'easy-hotel' ),
              'capitalize' => esc_html__( 'Capitalize', 'easy-hotel' ),
              'uppercase'  => esc_html__( 'Uppercase', 'easy-hotel' ),
              'lowercase'  => esc_html__( 'Lowercase', 'easy-hotel' )
            ), 'text-transform', esc_html__( 'Default', 'easy-hotel' ) ));
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_decoration'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Text Decoration', 'easy-hotel' ) .'</div>';
            echo esc_html($this->create_select( array(
              'none'               => esc_html__( 'None', 'easy-hotel' ),
              'underline'          => esc_html__( 'Solid', 'easy-hotel' ),
              'underline double'   => esc_html__( 'Double', 'easy-hotel' ),
              'underline dotted'   => esc_html__( 'Dotted', 'easy-hotel' ),
              'underline dashed'   => esc_html__( 'Dashed', 'easy-hotel' ),
              'underline wavy'     => esc_html__( 'Wavy', 'easy-hotel' ),
              'underline overline' => esc_html__( 'Overline', 'easy-hotel' ),
              'line-through'       => esc_html__( 'Line-through', 'easy-hotel' )
            ), 'text-decoration', esc_html__( 'Default', 'easy-hotel' ) ));
            echo '</div>';
          }
        ...
        echo '<div class="csf--blocks csf--blocks-inputs">';
          ...
          if ( ! empty( $args['font_size'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Font Size', 'easy-hotel' ) .'</div>';
          ...
          if ( ! empty( $args['line_height'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Line Height', 'easy-hotel' ) .'</div>';
            echo '<div class="csf--input-wrap">';
          ...
          if ( ! empty( $args['letter_spacing'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Letter Spacing', 'easy-hotel' ) .'</div>';
            echo '<div class="csf--input-wrap">';
          ...
          if ( ! empty( $args['word_spacing'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Word Spacing', 'easy-hotel' ) .'</div>';
            echo '<div class="csf--input-wrap">';
        ...
        if ( ! empty( $args['color'] ) ) {
          ...
          echo '<div class="csf--title">'. esc_html__( 'Font Color', 'easy-hotel' ) .'</div>';
          echo '<div class="csf-field-color">';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '[color]' ) ) .'" class="csf-color csf--color" value="'. esc_attr( $this->value['color'] ) .'"'. esc_attr($default_color_attr) .' />';
          echo '</div>';
          echo '</div>';
        }
        ...
        if ( ! empty( $args['custom_style'] ) ) {
          ...
          echo '<div class="csf--title">'. esc_html__( 'Custom Style', 'easy-hotel' ) .'</div>';
    ...
    public function enqueue() {
      ...
      if ( ! wp_script_is( 'csf-webfontloader' ) ) {
        ...
        ESHB::include_plugin_file( 'fields/typography/google-fonts.php' );

        wp_enqueue_script( 'csf-webfontloader', ESHB_PL_PATH . 'admin/includes/framework/assets/js/webfontloader.min.js', array( 'easy-hotel' ), '1.6.28', true );

        $webfonts = array();

        $customwebfonts = apply_filters( 'eshb_field_typography_customwebfonts', array() );

        if ( ! empty( $customwebfonts ) ) {
          $webfonts['custom'] = array(
            'label' => esc_html__( 'Custom Web Fonts', 'easy-hotel' ),
            'fonts' => $customwebfonts
          );
        }

        $webfonts['safe'] = array(
          'label' => esc_html__( 'Safe Web Fonts', 'easy-hotel' ),
          'fonts' => apply_filters( 'eshb_field_typography_safewebfonts', array(
            'Arial',
            'Arial Black',
            'Helvetica',
            'Times New Roman',
        ...
        $webfonts['google'] = array(
          'label' => esc_html__( 'Google Web Fonts', 'easy-hotel' ),
          'fonts' => apply_filters( 'eshb_field_typography_googlewebfonts', eshb_get_google_fonts()
        ) );

        $defaultstyles = apply_filters( 'eshb_field_typography_defaultstyles', array( 'normal', 'italic', '700', '700italic' ) );

        $googlestyles = apply_filters( 'eshb_field_typography_googlestyles', array(
          '100'       => 'Thin 100',
        ...
        $webfonts = apply_filters( 'eshb_field_typography_webfonts', $webfonts );

        wp_localize_script( 'easy-hotel', 'eshb_typography_json', array(
          'webfonts'      => $webfonts,
    ...
    public function enqueue_google_fonts( $method = 'enqueue' ) {
      ...
      if ( ! empty( $this->value['type'] ) ) {
        ...
        ESHB::include_plugin_file( 'fields/typography/google-fonts.php' );
        $is_google = ( array_key_exists( $this->value['font-family'], eshb_get_google_fonts() ) ) ? true : false;
      }
      ...
      if ( $is_google ) {
        ...
        if ( $font_weight || $font_style ) {
          ...
          if ( ! empty( $style ) ) {
            ...
            ESHB::$webfonts[$method][$font_family][$style] = $style;
          }
        } else {
          ESHB::$webfonts[$method][$font_family] = array();
        }
        ...
        if ( ! empty( $this->value['extra-styles'] ) ) {
          foreach ( $this->value['extra-styles'] as $extra_style ) {
            if ( ! empty( $extra_style ) ) {
              ...
              ESHB::$webfonts[$method][$font_family][$extra_style] = $extra_style;
            }
          }
        }
