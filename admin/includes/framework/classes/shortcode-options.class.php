 ...
 *
 */
if ( ! class_exists( 'ESHB_Shortcoder' ) ) {
  class ESHB_Shortcoder extends ESHB_Abstract{
    ...
    public $args         = array(
      ...
      'gutenberg'        => array(
        ...
        'icon'           => 'screenoptions',
        'category'       => 'widgets',
        'keywords'       => array( 'shortcode', 'easy-hotel', 'insert' ),
    ...
    public function __construct( $key, $params = array() ) {
      ...
      if ( ! empty( $this->args['show_in_editor'] ) ) {
        ...
        ESHB::$shortcode_instances[] = wp_parse_args( array( 'name' => 'csf/'. $name, 'modal_id' => $this->unique ), $this->args );

        // elementor editor support
        if ( ESHB::is_active_plugin( 'elementor/elementor.php' ) ) {
          add_action( 'elementor/editor/before_enqueue_scripts', array( 'ESHB', 'add_admin_enqueue_scripts' ) );
          add_action( 'elementor/editor/footer', array( 'ESHB_Field_icon', 'add_footer_modal_icon' ) );
    ...
    ?>
      <div id="csf-modal-<?php echo esc_attr( $this->unique ); ?>" class="wp-core-ui csf-modal csf-shortcode hidden<?php echo esc_attr( $single_usage . $class ); ?>" data-modal-id="<?php echo esc_attr( $this->unique ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'eshb_shortcode_nonce' ) ); ?>">
                ...
                foreach ( $this->pre_tabs as $tab ) {
                  ...
                  if ( ! empty( $tab['subs'] ) ) {
                    ...
                    foreach ( $tab['subs'] as $sub ) {
                      ...
                      echo '<option value="'. esc_attr( $tab_key ) .'"'. esc_attr($view) . esc_attr($shortcode) . esc_attr($group) .'>'. esc_attr( $sub['title'] ) .'</option>';
    ...
    public function get_shortcode() {
      ...
      if ( ! empty( $shortcode_key ) && wp_verify_nonce( $nonce, 'eshb_shortcode_nonce' ) ) {
        ...
        if ( ! empty( $section ) ) {
          ...
          if ( ! empty( $section['fields'] ) && $view !== 'repeater' ) {
            ...
            foreach ( $section['fields'] as $field ) {
              ...
              ESHB::field( $field, $field_default, $shortcode, 'shortcode' );
          ...
          if ( ! empty( $repeatable_fields ) ) {
            ...
            $button_title    = ( ! empty( $section['button_title'] ) ) ? ' '. $section['button_title'] : esc_html__( 'Add New', 'easy-hotel' );
            ...
            echo '<div class="csf--repeatable">';
              ...
              echo '<div class="csf--repeat-shortcode">';
                ...
                foreach ( $repeatable_fields as $field ) {
                  ...
                  ESHB::field( $field, $field_default, $inner_shortcode.'[0]', 'shortcode' );
            ...
            echo '<div class="csf--repeat-button-block"><a class="button csf--repeat-button" href="#"><i class="fas fa-plus-circle"></i> '. esc_html($button_title) .'</a></div>';
        ...
        echo '<div class="csf-field csf-error-text">'. esc_html__( 'Error: Invalid nonce verification.', 'easy-hotel' ) .'</div>';
      }
    ...
    public static function once_editor_setup() {
      ...
      if ( function_exists( 'register_block_type' ) ) {
        add_action( 'enqueue_block_editor_assets', array( 'ESHB_Shortcoder', 'add_guteberg_blocks' ) );
      }

      if ( eshb_wp_editor_api() ) {
        add_action( 'media_buttons', array( 'ESHB_Shortcoder', 'add_media_buttons' ) );
      }
    ...
    public static function add_guteberg_blocks() {
      ...
      wp_enqueue_script( 'csf-gutenberg-block', ESHB::include_plugin_url( 'assets/js/gutenberg.js' ), $depends, ESHB_VERSION, true );

      wp_localize_script( 'csf-gutenberg-block', 'eshb_gutenberg_blocks', ESHB::$shortcode_instances );

      foreach ( ESHB::$shortcode_instances as $block ) {
    ...
    public static function add_media_buttons( $editor_id ) {
      ...
      foreach ( ESHB::$shortcode_instances as $value ) {
