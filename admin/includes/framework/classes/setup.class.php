...
if ( ! class_exists( 'ESHB_Setup' ) ) {
  class ESHB_Setup {
    ...
    public function __construct() {
      ...
      add_action( 'after_setup_theme', array( 'ESHB', 'setup' ) );
      add_action( 'init', array( 'ESHB', 'setup' ) );
      add_action( 'switch_theme', array( 'ESHB', 'setup' ) );
      add_action( 'admin_enqueue_scripts', array( 'ESHB', 'add_admin_enqueue_scripts' ) );
      add_action( 'wp_enqueue_scripts', array( 'ESHB', 'add_typography_enqueue_styles' ), 80 );
      add_action( 'wp_head', array( 'ESHB', 'add_custom_css' ), 80 );
      add_filter( 'admin_body_class', array( 'ESHB', 'add_admin_body_class' ) );
    ...
    public static function setup() {
      ...
      $params = array();
      if ( class_exists( 'ESHB_Options' ) && ! empty( self::$args['admin_options'] ) ) {
        foreach ( self::$args['admin_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ESHB_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ESHB_Customize_Options' ) && ! empty( self::$args['customize_options'] ) ) {
        foreach ( self::$args['customize_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ESHB_Customize_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ESHB_Metabox' ) && ! empty( self::$args['metabox_options'] ) ) {
        foreach ( self::$args['metabox_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ESHB_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ESHB_Nav_Menu_Options' ) && ! empty( self::$args['nav_menu_options'] ) ) {
        foreach ( self::$args['nav_menu_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ESHB_Nav_Menu_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ESHB_Profile_Options' ) && ! empty( self::$args['profile_options'] ) ) {
        foreach ( self::$args['profile_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ESHB_Profile_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ESHB_Taxonomy_Options' ) && ! empty( self::$args['taxonomy_options'] ) ) {
        ...
        wp_verify_nonce( sanitize_text_field(wp_unslash($_POST['eshb_save_meta'])), 'eshb_save_meta_box_nonce');
          ...
          foreach ( self::$args['taxonomy_options'] as $key => $value ) {
            if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
              ...
              ESHB_Taxonomy_Options::instance( $key, $params );
      ...
      if ( class_exists( 'ESHB_Widget' ) && class_exists( 'WP_Widget_Factory' ) && ! empty( self::$args['widget_options'] ) ) {
        ...
        foreach ( self::$args['widget_options'] as $key => $value ) {
          if ( ! isset( self::$inited[$key] ) ) {
            ...
            self::$inited[$key] = true;
            $wp_widget_factory->register( ESHB_Widget::instance( $key, $value ) );
      ...
      $params = array();
      if ( class_exists( 'ESHB_Comment_Metabox' ) && ! empty( self::$args['comment_options'] ) ) {
        foreach ( self::$args['comment_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ESHB_Comment_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ESHB_Shortcoder' ) && ! empty( self::$args['shortcode_options'] ) ) {
        foreach ( self::$args['shortcode_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ESHB_Shortcoder::instance( $key, $params );

          }
        }

        // Once editor setup for gutenberg and media buttons
        if ( class_exists( 'ESHB_Shortcoder' ) && ! empty( self::$shortcode_instances ) ) {
          foreach ( self::$shortcode_instances as $instance ) {
            if ( ! empty( $instance['show_in_editor'] ) ) {
              ESHB_Shortcoder::once_editor_setup();
              break;
            }
          }
        }
    ...
    public static function include_plugin_file( $file, $load = true ) {
      ...
      $path     = '';
      $file     = ltrim( $file, '/' );
      $override = apply_filters( 'eshb_override', 'csf-override' );
    ...
    public static function includes() {
      ...
      $fields = apply_filters( 'eshb_fields', array(
        'accordion',
        'background',
        'backup',
        'border',
      ...
      if ( ! empty( $fields ) ) {
        foreach ( $fields as $field ) {
          if ( ! class_exists( 'ESHB_Field_'. $field ) && class_exists( 'ESHB_Fields' ) ) {
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      if ( ! apply_filters( 'eshb_enqueue_assets', self::$enqueue ) ) {
        return;
      }
      ...
      if ( apply_filters( 'eshb_fa4', false ) ) {
        wp_enqueue_style( 'csf-fa', self::include_plugin_url( 'assets/css/font-awesome.min.css'), array(), '4.7.0', 'all' );
      } else {
        wp_enqueue_style( 'csf-fa5', self::include_plugin_url( 'assets/css/fa5-all.min.css'), array(), '5.15.5', 'all' );
      ...
      // Main style
      wp_enqueue_style( 'easy-hotel', self::include_plugin_url( 'assets/css/style'. $min .'.css' ), array(), self::$version, 'all' );
      ...
      wp_enqueue_script( 'easy-hotel', self::include_plugin_url( 'assets/js/main'. $min .'.js' ), array( 'csf-plugins' ), self::$version, true );

      // Main variables
      wp_localize_script( 'easy-hotel', 'eshb_vars', array(
        'color_palette'     => apply_filters( 'eshb_color_palette', array() ),
        'i18n'              => array(
          'confirm'         => esc_html__( 'Are you sure?', 'easy-hotel' ),
          'typing_text'     => esc_html__( 'Please enter or more characters', 'easy-hotel' ),
          'searching_text'  => esc_html__( 'Searching...', 'easy-hotel' ),
          'no_results_text' => esc_html__( 'No results found.', 'easy-hotel' ),
        ),
      ) );
      ...
      if ( ! empty( self::$fields ) ) {
        foreach ( self::$fields as $field ) {
          if ( ! empty( $field['type'] ) ) {
            $classname = 'ESHB_Field_' . $field['type'];
    ...
    public static function add_custom_css() {
      ...
      if ( ! empty( self::$css ) ) {
          $version = defined( 'ESH_PLUGIN_VERSION' ) ? ESH_PLUGIN_VERSION : time();

          wp_enqueue_style(
              'eshb-custom-style',
              false,
              array(),      // Dependencies (empty since no external stylesheet)
    ...
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
      ...
      if ( ! empty( $field['_notice'] ) ) {
        ...
        $field            = array();
        $field['content'] = esc_html__( 'Oops! Not allowed.', 'easy-hotel' ) .' <strong>('. $field_type .')</strong>';
        $field['type']    = 'notice';
        $field['style']   = 'danger';
      ...
      echo '<div class="csf-field csf-field-'. esc_attr($field_type) . esc_attr($is_pseudo) . esc_attr($class) . esc_attr($visible) .'"'. esc_attr($depend) .'>';
      ...
      if ( ! empty( $field_type ) ) {
        ...
        $classname = 'ESHB_Field_'. $field_type;

        if ( class_exists( $classname ) ) {
          $instance = new $classname( $field, $value, $unique, $where, $parent );
          $instance->render();
        } else {
          echo '<p>'. esc_html__( 'Field not found!', 'easy-hotel' ) .'</p>';
        }
