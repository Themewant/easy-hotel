 ...
 *
 */
if ( ! class_exists( 'ESHB_Comment_Metabox' ) ) {
  class ESHB_Comment_Metabox extends ESHB_Abstract{
    ...
    public function add_comment_meta_box_content( $comment, $callback ) {
      ...
      $errors   = ( is_object ( $comment ) ) ? get_comment_meta( $comment->comment_ID, '_eshb_errors_'. $this->unique, true ) : array();
      $errors   = ( ! empty( $errors ) ) ? $errors : array();
      $theme    = ( $this->args['theme'] ) ? ' csf-theme-'. $this->args['theme'] : '';
      $nav_type = ( $this->args['nav'] === 'inline' ) ? 'inline' : 'normal';

      if ( is_object( $comment ) && ! empty( $errors ) ) {
        delete_comment_meta( $comment->comment_ID, '_eshb_errors_'. $this->unique );
      }

      wp_nonce_field( 'eshb_comment_metabox_nonce', 'eshb_comment_metabox_nonce'. $this->unique );
        ...
        echo '<div class="csf-wrapper'. esc_attr( $show_all ) .'">';
          ...
          echo '<div class="csf-content">';
            ...
            foreach ( $this->sections as $section ) {
              ...
              echo ( $section_title || $section_icon ) ? '<div class="csf-section-title"><h3>' . wp_kses_post( $section_icon ) . esc_html( $section_title ) . '</h3></div>' : '';
              ...
              if ( ! empty( $section['fields'] ) ) {
                ...
                foreach ( $section['fields'] as $field ) {
                  ...
                  ESHB::field( $field, $this->get_meta_value( $comment->comment_ID, $field ), $this->unique, 'comment_metabox' );
            ...
            if ( ! empty( $this->args['show_restore'] ) || ! empty( $this->args['show_reset'] ) ) {
              ...
              echo '<span class="button csf-button-reset">'. esc_html__( 'Reset', 'easy-hotel' ) .'</span>';
              echo '<span class="button csf-button-cancel">'. sprintf( '<small>( %s )</small> %s', esc_html__( 'update post', 'easy-hotel' ), esc_html__( 'Cancel', 'easy-hotel' ) ) .'</span>';
              echo '</label>';
              echo '</div>';
    ...
    public function save_comment_meta_box( $comment_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'eshb_comment_metabox_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'eshb_comment_metabox_nonce' ) ) {
        return $comment_id;
      }

      // XSS ok.
      // No worries, This "POST" requests is sanitizing in the below foreach.
      $request = ( ! empty( $_POST[ $this->unique ] ) ) ? sanitize_text_field( wp_unslash($_POST[ $this->unique ]) ) : array();
      ...
      $data = apply_filters( "eshb_{$this->unique}_save", $data, $comment_id, $this );

      do_action( "eshb_{$this->unique}_save_before", $data, $comment_id, $this );
      ...
      if ( empty( $data ) || ! empty( $request['_reset'] ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_comment_meta( $comment_id, '_eshb_errors_'. $this->unique, $errors );
        }

      }

      do_action( "eshb_{$this->unique}_saved", $data, $comment_id, $this );

      do_action( "eshb_{$this->unique}_save_after", $data, $comment_id, $this );
