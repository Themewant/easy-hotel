 ...
 *
 */
if ( ! class_exists( 'ESHB_Options' ) ) {
  class ESHB_Options extends ESHB_Abstract {
    ...
    public function __construct( $key, $params = array() ) {
      ...
      $this->unique   = $key;
      $this->args     = apply_filters( "eshb_{$this->unique}_args", wp_parse_args( $params['args'], $this->args ), $this );
      $this->sections = apply_filters( "eshb_{$this->unique}_sections", $params['sections'], $this );
      ...
      add_action( 'wp_ajax_eshb_'. $this->unique .'_ajax_save', array( $this, 'ajax_save' ) );
    ...
    public function ajax_save() {
      ...
      if ( ! $result ) {
        wp_send_json_error( array( 'error' => esc_html__( 'Error while saving the changes.', 'easy-hotel' ) ) );
      } else {
    ...
    public function set_options( $ajax = false ) {
      ...
      //$response  = ( $ajax && ! empty( $_POST['data'] ) ) ? array_map( 'sanitize_text_field', json_decode( trim( wp_unslash($_POST['data']) ), true )) : $_POST;
      
      $response = array();

      if ( $ajax && ! empty( $_POST['data'] ) ) {
          // Sanitize the entire $_POST['data'] input before decoding
          $post_data = sanitize_text_field( wp_unslash( $_POST['data'] ) );
          
          // Decode sanitized data and apply further sanitization via array_map
          $response = json_decode( trim( $post_data ), true );
      } 
      // else {
      //     // If there's no 'data' in the post, use the whole sanitized $_POST array
      //     $response = array_map( 'sanitize_text_field', $_POST );
      // }
      ...
      // Set variables.
      $data      = array();
      $noncekey  = 'eshb_options_nonce'. $this->unique;
      $nonce     = ( ! empty( $response[$noncekey] ) ) ? $response[$noncekey] : '';
      $options   = ( ! empty( $response[$this->unique] ) ) ? $response[$this->unique] : array();
      $transient = ( ! empty( $response['eshb_transient'] ) ) ? $response['eshb_transient'] : array();

      if ( wp_verify_nonce( $nonce, 'eshb_options_nonce' ) ) {

        $importing  = false;
        $section_id = ( ! empty( $transient['section'] ) ) ? $transient['section'] : '';

        if ( ! $ajax && ! empty( $response[ 'eshb_import_data' ] ) ) {

          // XSS ok.
          // No worries, This "POST" requests is sanitizing in the below foreach. see #L337 - #L341
          $import_data  = json_decode( wp_unslash( trim( $response[ 'eshb_import_data' ] ) ), true );
          $options      = ( is_array( $import_data ) && ! empty( $import_data ) ) ? $import_data : array();
          $importing    = true;
          $this->notice = esc_html__( 'Settings successfully imported.', 'easy-hotel' );

        }

        if ( ! empty( $transient['reset'] ) ) {
          $pre_fields = array_map('sanitize_text_field', $this->pre_fields);
          foreach ( $this->pre_fields as $field ) {
            if ( ! empty( $field['id'] ) ) {
          ...
          $this->notice = esc_html__( 'Default settings restored.', 'easy-hotel' );
        ...
        $data = apply_filters( "eshb_{$this->unique}_save", $data, $this );

        do_action( "eshb_{$this->unique}_save_before", $data, $this );

        $this->options = $data;

        $this->save_options( $data );

        do_action( "eshb_{$this->unique}_save_after", $data, $this );

        if ( empty( $this->notice ) ) {
          $this->notice = esc_html__( 'Settings saved.', 'easy-hotel' );
        }
    ...
    public function save_options( $data ) {
      ...
      do_action( "eshb_{$this->unique}_saved", $data, $this );
    ...
    public function add_options_html() {
      ...
      echo '<div class="csf csf-options'. esc_attr( $theme . $class . $wrapper_class ) .'" data-slug="'. esc_attr( $this->args['menu_slug'] ) .'" data-unique="'. esc_attr( $this->unique ) .'">';
        ...
        echo '<input type="hidden" class="csf-section-id" name="eshb_transient[section]" value="1">';

        wp_nonce_field( 'eshb_options_nonce', 'eshb_options_nonce'. $this->unique );
        ...
        echo '<div class="csf-header-inner">';
          ...
          echo '<div class="csf-header-right">';
            ...
            echo ( $this->args['show_form_warning'] ) ? '<div class="csf-form-result csf-form-warning">'. esc_html__( 'You have unsaved changes, save your changes!', 'easy-hotel' ) .'</div>' : '';

            echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="csf-expand-all" title="'. esc_html__( 'show all settings', 'easy-hotel' ) .'"><i class="fas fa-outdent"></i></div>' : '';

            echo ( $this->args['show_search'] ) ? '<div class="csf-search"><input type="text" name="csf-search" placeholder="'. esc_html__( 'Search...', 'easy-hotel' ) .'" autocomplete="off" /></div>' : '';
            ...
            echo '<div class="csf-nav csf-nav-'. esc_attr( $nav_type ) .' csf-nav-options">';
              ...
              foreach ( $this->pre_tabs as $tab ) {
                ...
                if ( ! empty( $tab['subs'] ) ) {
                  ...
                  echo '<a href="#tab=' . esc_attr( $tab_id ) . '" data-tab-id="' . esc_attr( $tab_id ) . '" class="csf-arrow">' 
                  . wp_kses_post($tab_icon)  // Assuming $tab_icon contains HTML
                  . esc_html( $tab['title'] ) 
                  . esc_html( $tab_error ) 
                  . '</a>';
                  ...
                  echo '<li class="csf-tab-item"><a href="#tab='. esc_attr( $tab_id ) .'" data-tab-id="'. esc_attr( $tab_id ) .'">'. wp_kses_post($tab_icon) . esc_html($tab['title']) . esc_html($tab_error) .'</a></li>';
            ...
            foreach ( $this->pre_sections as $section ) {
              ...
              if ( ! empty( $section['fields'] ) ) {
                ...
                foreach ( $section['fields'] as $field ) {
                  ...
                  ESHB::field( $field, $value, $this->unique, 'options' );

                }

              } else {

                echo '<div class="csf-no-option">'. esc_html__( 'No data available.', 'easy-hotel' ) .'</div>';
