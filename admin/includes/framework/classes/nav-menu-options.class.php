 ...
 *
 */
if ( ! class_exists( 'ESHB_Nav_Menu_Options' ) ) {
  class ESHB_Nav_Menu_Options extends ESHB_Abstract{
    ...
    public function wp_edit_nav_menu_walker( $class, $menu_id ) {
      ...
      if( version_compare( $wp_version, '5.4.0', '<' ) ) {
        ...
        if ( ! class_exists( 'ESHB_Walker_Nav_Menu_Edit' ) ) {
          ESHB::include_plugin_file( 'functions/walker.php' );
        }

        return 'ESHB_Walker_Nav_Menu_Edit';
    ...
    public function wp_nav_menu_item_custom_fields( $menu_item_id, $item, $depth, $args ) {
      ...
      $errors = ( ! empty( $menu_item_id ) ) ? get_post_meta( $menu_item_id, '_eshb_errors_'. $this->unique, true ) : array();
      $errors = ( ! empty( $errors ) ) ? $errors : array();
      $class  = ( $this->args['class'] ) ? ' '. $this->args['class'] : '';

      if ( ! empty( $errors ) ) {
        delete_post_meta( $menu_item_id, '_eshb_errors_'. $this->unique );
      }
      ...
      echo '<div class="csf csf-nav-menu-options'. esc_attr( $class ) .'">';
        ...
        foreach ( $this->sections as $section ) {
          ...
          echo ( $section_title || $section_icon ) ? '<div class="csf-nav-menu-title"><h4>'. wp_kses_post($section_icon) . esc_html($section_title) .'</h4></div>' : '';
          ...
          if ( ! empty( $section['fields'] ) ) {
            ...
            foreach ( $section['fields'] as $field ) {
              ...
              ESHB::field( $field, $this->get_meta_value( $menu_item_id, $field ), $this->unique .'['. $menu_item_id .']', 'menu' );
    ...
    public function wp_update_nav_menu_item( $menu_id, $menu_item_db_id, $menu_item_args ) {
      ...
      $request = ( ! empty( $_POST[ $this->unique ][ $menu_item_db_id ] ) ) ? sanitize_text_field( wp_unslash($_POST[ $this->unique ]))[ $menu_item_db_id ] : array();
      ...
      $data = apply_filters( "eshb_{$this->unique}_save", $data, $menu_item_db_id, $this );

      do_action( "eshb_{$this->unique}_save_before", $data, $menu_item_db_id, $this );
      ...
      if ( empty( $data ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_post_meta( $menu_item_db_id, '_eshb_errors_'. $this->unique, $errors );
        }

      }

      do_action( "eshb_{$this->unique}_saved", $data, $menu_item_db_id, $this );

      do_action( "eshb_{$this->unique}_save_after", $data, $menu_item_db_id, $this );
