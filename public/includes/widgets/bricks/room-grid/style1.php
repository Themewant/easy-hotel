<?php 
	...
	$i++; 
	$eshb_class = ($i %2 == 0) ? 'left-half' : 'right-half';
	
	if(has_post_thumbnail($accomodation_id)) {
		$eshb_thumbnail_url = get_the_post_thumbnail_url( $accomodation_id, 'full');
	} else {
	...
	<div class="grid-item container-fluid position-relative half-fluid" style="background-size: cover; background-repeat: no-repeat;">
		...
		<div class="eshb-row" style="background-size: cover; background-repeat: no-repeat;">
			<!-- Image -->
			<div class="thumbnail-col col-lg-6 col-sm-12 col-12 position-lg-absolute <?php echo esc_attr( $eshb_class )?> eshb-h-100" style="background-size: cover; background-repeat: no-repeat;">
				<div class="image bgcustom" data-bgimage="url(<?php echo esc_attr($eshb_thumbnail_url) ?>) center" style="background: url(<?php echo esc_url($eshb_thumbnail_url) ?>) center center / cover no-repeat;"></div>
			</div>
			<!-- Text -->
				...
				<div class="capacities fs-14 mb-3" style="background-size: cover; background-repeat: no-repeat;">
					<?php 
						if ( ! empty( $accomodation_info_group ) && is_array($accomodation_info_group) && count($accomodation_info_group) > 0) {
							$eshb_x = 0;
							foreach ( $accomodation_info_group as $eshb_group ) { 
								$eshb_x++;
								if($eshb_x >= 3) break;
								?>
